...
from data_utils.imagenet import imagenet_data
...
IMAGENET_PATH = 'D:\Workspace\Scripts\datasets\Sample_1000'
...
def make_adversarial_examples(image, true_label, args, model_to_fool, IMAGENET_SL):
    ...
    while not ch.any(total_queries > args.max_queries):
        ...
        if args.log_progress:
            print("Queries: %d | Success rate: %f | Average queries: %f | Average norm: %f" % (
            max_curr_queries, current_success_rate, success_queries, ch.norm(image-orig_images)/orig_images.shape[0]))

        if current_success_rate == 1.0:
            break

    def l2_norm(img_a, img_b):
        return ch.norm(img_a-img_b)
    def l2_perturb(img_a, img_b):
        return l2_norm(img_a, img_b)/ch.norm(img_b)
    return {
        ...
        'l2_norms': [l2_norm(image[i], orig_images[i]) for i in range(image.shape[0])],
        'l2_perturb': [l2_perturb(image[i], orig_images[i]) for i in range(image.shape[0])],
...
def main(args, model_to_fool, dataset_size):
    ...
    # transform = transforms.Compose([transforms.Resize(dataset_size),
    #                     transforms.CenterCrop(dataset_size),
    #                     transforms.ToTensor(),
    #                     ])
    # dataset_loader = imagenet_data(transform=transform, batch_size=args.batch_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--json-config', type=str, help='a config file to be passed in instead of arguments', default='src/configs/l2.json')
    parser.add_argument('--epsilon', type=float, help='the lp perturbation bound')
    parser.add_argument('--batch-size', type=int, help='batch size for bandits')
    parser.add_argument('--log-progress', action='store_true', default=True)
    ...
    parser.add_argument('--classifier', type=str, default='resnet18', choices=CLASSIFIERS.keys())
